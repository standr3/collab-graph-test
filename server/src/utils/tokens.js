...
export function setRefreshCookie(res, userId) {
  const token = jwt.sign({ sub: String(userId) }, process.env.REAL_REFRESH_SECRET || process.env.REFRESH_SECRET, {
    expiresIn: '30d', issuer: 'your-api'
  });
...
export function clearRefreshCookie(res) {
  ...
  res.clearCookie('rt_demo', { httpOnly: false, secure: isProd, sameSite: 'lax', path: '/' });
}

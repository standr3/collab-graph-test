...
function ensureRefreshed() {
  if (!refreshPromise) {
    refreshPromise = API.post('/auth/refresh')
      .then(r => { const t = r.data?.access_token; if (t) setAccess(t); console.log('[REFRESH] ok'); })
      .catch(() => { setAccess(''); })
...
API.interceptors.response.use(
  ...
  async err => {
    ...
    if (status === 401 && (msg === 'TokenExpired' || (!getAccess() && msg === 'Unauthorized'))) {
...
export async function rehydrateAccess() {
  try { const { data } = await API.post('/auth/refresh'); setAccess(data?.access_token || ''); return !!data?.access_token; }
  catch { setAccess(''); return false; }
}
